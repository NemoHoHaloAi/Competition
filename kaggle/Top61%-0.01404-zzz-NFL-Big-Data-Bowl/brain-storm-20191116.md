...
Play信息：
进攻组、防守组拆分为多个位置的人数字段，也避免了label导致的异常；
...
这个数据不一定有是个问题，目前测试数据是22条的给的，也就是没法构建太多需要上下文的特征，那么能操作的也就是这22条；

每节结束后，双方换边；

弃踢：如果踢出界，那么对方开球点会比较靠前，因此应该是尽量靠近对方达阵区，但是不出界；

是否到了goal区，在这个区域，是不按照10码要求定的，必须4次内完成得分，达阵或者射门，一般down4还是有一定距离则选择射门拿3分，放弃达阵6分；
```
